...
class ProductRouting {
    ...
    static showHome(req, res) {
        fs.readFileSync('./views/home.html', 'utf-8', async (err, homeHtml) => {
            if (err) {
                console.log(err);
            }
            else {
    ...
    static createProduct(req, res) {
        if (req.method === 'GET') {
            fs.readFileSync('./views/create.html', 'utf-8', async (err, createHtml) => {
                if (err) {
                    console.log(err);
                }
                else {
    ...
    static async deleteProduct(req, res, id) {
        if (req.method === 'GET') {
            fs.readFileSync('./views/delete.html', 'utf-8', async (err, removeHtml) => {
                if (err) {
                    console.log(err);
                }
                else {
