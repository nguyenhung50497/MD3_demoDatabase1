        ...
        if (req.method === 'GET') {
            ...
            req.on('end', err => {
                if (err) {
                    ...
                    const product = qs.parse(data);
                    const mess = await productService.findByNameContaining(product);
                    console.log(mess);
                    fs.readFileSync('./views/search.html', 'utf-8', async (err, searchHtml) => {
                        if (err) {
                            console.log(err);
                        }
                        else {
                            searchHtml = searchHtml.replace('{products}', product)
                            res.writeHead(200, 'text/html');
                            res.write(searchHtml);
                            res.end();
                        }
