...
class ProductRouting {
    ...
    static searchProduct(req, res, id) {
        if (req.method === 'GET') {
            fs.readFile('./views/search.html', 'utf-8', async (err, searchHtml) => {
                if (err) {
                    console.log(err);
                }
                else {
            ...
            req.on('end', async err => {
                if (err) {
                    ...
                    const mess = await productService.findByNameContaining(products.name);
                    await fs.readFileSync('./views/search.html', 'utf-8', (err, searchHtml) => {
                        if (err) {
                            console.log(err);
                        }
                        else {
                            console.log(1);
                            searchHtml = searchHtml.replace('{products}', 'adkjadkjashdahkdjsaj')
                            res.writeHead(200, 'text/html');
                            res.write(searchHtml);
                            res.end();
                        }
